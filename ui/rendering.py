...
class ScreenRenderer:
    ...
    def draw_background(self, image_path: Optional[str] = None, glass_alpha: int = 180) -> None:
        """
        ...
        try:
            ...
            bg_rect = self.canvas.create_rectangle(
                0, 0, self.screen_width, self.screen_height, fill=bg_color, outline="", tags="bg"
            )
    ...
    def draw_header(
        self, clock_text: str = "00:00:00", network_icon: str = "●", title_text: str = "DEDSEC"
    ) -> None:
        """
    ...
    def draw_terminal(
        ...
        try:
            ...
            term_rect = self.canvas.create_rectangle(
                ...
                LAYOUT.HEADER_HEIGHT,
                self.screen_width,
                LAYOUT.CANVAS_HEIGHT - LAYOUT.STATUS_BAR_HEIGHT,
                fill=term_bg,
                outline=self._get_color("border"),
                tags="terminal_bg",
            )
            ...
            for idx, line in enumerate(log_lines):
                ...
                if (
                    LAYOUT.HEADER_HEIGHT - 10
                    < y_pos
                    < (LAYOUT.CANVAS_HEIGHT - LAYOUT.STATUS_BAR_HEIGHT) + 10
                ):
                    # Use pool if available, else create directly
                    if pool:
                        item_id = pool.acquire(
                            LAYOUT.SIDEBAR_WIDTH + 5,
                            y_pos,
                            text=f"> {line}",
                            fill=text_color,
                            font=("Courier", 9),
...
def create_default_renderer(
    canvas: tk.Canvas, theme_manager: Optional[ThemeManager] = None, debug: bool = False
) -> ScreenRenderer:
    """
    ...
    """
    context = RenderContext(canvas=canvas, theme_manager=theme_manager, debug_mode=debug)
