...
class ToolManager:
    ...
    def register_tool(self, tool: ToolMetadata) -> bool:
        ...
        try:
            ...
            if tool.requires_dependencies:
                ...
                if missing:
                    logger.warning(f"Tool {tool.id} missing dependencies: {missing}")
                    tool.enabled = False
    ...
    def register_lazy_tool(self, tool_id: str, loader: Callable[[], ToolMetadata]) -> bool:
        """
    ...
    def get_categories(self) -> List[ToolCategory]:
        ...
        """
        return [category for category, tools in self._category_index.items() if len(tools) > 0]
    ...
    def execute_tool(self, tool_id: str, **kwargs: Any) -> Any:
        ...
        context = ToolExecutionContext(
            tool_id=tool_id, status=ToolStatus.RUNNING, start_time=time.time()
        )
        ...
        try:
            ...
            # Audit log
            audit_log("TOOL_EXECUTE_FAILURE", tool_id=tool_id, error=str(e))
