...
class ColorGradient(Animator):
    ...
    def __init__(
        self, start_color: str, end_color: str, duration_frames: int = 30, loop: bool = False
    ):
        """
...
class FadeTransition(Animator):
    ...
    def __init__(self, fade_in: bool = True, duration_frames: int = 30, loop: bool = False):
        """
...
def create_button_press_animation() -> ColorGradient:
    ...
    return ColorGradient(start_color="#ccff00", end_color="#ffffff", duration_frames=5, loop=False)


def create_status_pulse() -> PulsingEffect:
    """Create pulsing effect for status indicators."""
    return PulsingEffect(min_value=0.6, max_value=1.0, duration_frames=40, loop=True)
...
def create_matrix_background() -> MatrixRain:
    ...
    return MatrixRain(max_chars=8, duration_frames=150, loop=True)
